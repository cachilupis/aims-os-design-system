...
**Identity chips are stable attributes ONLY — never a dynamic state or metric.** Role/department/location, tier/segment/industry, company/deal value/lead source — yes. Adoption level, deal stage, health score — no, those change and don't pass the key-field filter either (see below) — they're removed from the header entirely (see the `// TODO: pertenece al Overview/tab de detalle` comments in record-header.tsx) and surface via Signal instead when urgent.

**RecordHeader is 3 content zones (Identity / Signal / Actions + key fields), not a fixed field list — see the Reference tab's "Content contract" section for the full framing.** Zone 3's key fields pass a glanceable+actionable filter into exactly 2 kinds, each mapped to an existing DS primitive: a contact action (Email, Phone) → `Button variant="tertiary"` with a leading icon; a relational link (Manager, Owner) → `TableCellLink` (table.tsx's real "Link-text=Yes" DS variant). A field with no click destination (Start date, MRR, Access role, ...) fails the filter and isn't shown in the header at all — it belongs on the Overview/detail tab. Key fields are always visible — no expand/collapse disclosure (Highlights Panel fields never hide behind a click).

**Don't invent chips, key fields, or action labels per screen** — pull them from the component's own exports so every instance stays predictable:
- `getRecordFields(variant, data)` → which fields become the 3 identity chips vs. Zone 3's key fields (see record-header.tsx or the Reference tab for the exact per-variant list)
